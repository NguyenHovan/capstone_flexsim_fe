...
const WorkspaceManager: React.FC = () => {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setWorkspaces(workspaces.map(ws => ws.id === id ? updatedWorkspace : ws));
  ...
  const columns = [
    ...
    { title: 'Image URL', dataIndex: 'imgUrl', key: 'imgUrl', render: (url: string) => url ? <a href={url} target="_blank" rel="noopener noreferrer">View</a> : 'N/A' },
    { title: 'Description', dataIndex: 'description', key: 'description' },
    { title: 'Active', dataIndex: 'isActive', key: 'isActive', render: (isActive: boolean) => isActive ? 'Yes' : 'No' },
    { title: 'Created At', dataIndex: 'createdAt', key: 'createdAt', render: (text: string) => new Date(text).toLocaleDateString() },
    { title: 'Updated At', dataIndex: 'updatedAt', key: 'updatedAt', render: (text: string) => text ? new Date(text).toLocaleDateString() : 'N/A' },
    { title: 'Deleted At', dataIndex: 'deleteAt', key: 'deleteAt', render: (text: string) => text ? new Date(text).toLocaleDateString() : 'N/A' },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: any, record: Workspace) => (
        <div>
          <Button icon={<CheckOutlined />} style={{ marginRight: 8 }} onClick={() => handleApprove(record.id)} disabled={record.isActive}>
            Approve
          </Button>
          <Button icon={<CloseOutlined />} danger onClick={() => handleReject(record.id)} disabled={!record.isActive}>
            Reject
          </Button>
      ...
      >
        {selectedWorkspace && (
          <div>
            ...
            <p><strong>Image URL:</strong> {selectedWorkspace.imgUrl ? <a href={selectedWorkspace.imgUrl} target="_blank" rel="noopener noreferrer">View</a> : 'N/A'}</p>
