...
import {
  ...
  InboxOutlined, DownloadOutlined, InfoCircleOutlined
...
/** Tạo file Excel (.xls): bắt buộc userName, fullName, email, password; còn lại tuỳ chọn */
const buildExcelTemplateBlob = (kind: 'student' | 'instructor') => {
  const headers = ['userName','fullName','email','password','gender','phone','address'];
  const example = [
    ...
    'Abc@123','1','0901234567','Hanoi'
  ];
  const tableRows =
    `<tr>${headers.map(h => `<th style="border:1px solid #ddd;padding:6px;background:#fafbff">${h}</th>`).join('')}</tr>` +
    `<tr>${example.map(v => `<td style="border:1px solid #ddd;padding:6px">${v}</td>`).join('')}</tr>`;
  const html =
...
const TMPL_COLS: ColumnsType<TmplRow> = [
  ...
  { title: 'Required', dataIndex: 'required', key: 'required', width: 110,
...
const TEMPLATE_ROWS: TmplRow[] = [
  ...
  { name: 'gender',   required: false, example: '1 | 2 | 3 (1=Male, 2=Female, 3=Other)' },
  { name: 'phone',    required: false, example: '0901234567' },
  { name: 'address',  required: false, example: 'Hà Nội' },
];

const TemplateBox: React.FC<{ kind: 'student' | 'instructor'; onDownload: () => void; }> = ({ kind, onDownload }) => (
  <Card size="small" className="tmpl-card" style={{ marginBottom: 12 }}>
    <div className="tmpl-header">
      <div className="tmpl-title"><InfoCircleOutlined /> Template columns — {kind === 'student' ? 'Student' : 'Instructor'}</div>
      <Button icon={<DownloadOutlined />} onClick={onDownload}>Tải file mẫu (.xls)</Button>
    </div>
    <Table<TmplRow> size="small" rowKey="name" pagination={false} columns={TMPL_COLS} dataSource={TEMPLATE_ROWS} />
    <div className="tmpl-note">
      <div>• <b>Bắt buộc:</b> <code>userName</code>, <code>fullName</code>, <code>email</code>, <code>password</code></div>
      <div>• Chấp nhận import: <b>.xlsx/.xls</b>. Nếu dùng <code>gender</code>: 1=Male, 2=Female, 3=Other.</div>
    </div>
  </Card>
);
...
const UserOrganization: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const filtered = (all ?? []).filter((u: Account) => VISIBLE_ROLE_IDS.has(Number(u.roleId)));
      setUsers(filtered);
    } catch (err) {
  ...
  const loadUsser = async (opts?: { close?: 'edit' | 'instr' | 'stud' | 'all' }) => {
    ...
    switch (opts?.close) {
      ...
      case 'instr':
        ...
        setTabInstr('manual'); setInstrFile(null);
        break;
      case 'stud':
        setIsCreateStudent(false); formStud.resetFields(); setStudError(null);
        setTabStud('manual'); setStudFile(null);
        break;
      case 'all':
        setIsEditModal(false); setEditingUser(null); formEdit.resetFields(); setEditError(null);
        setIsCreateInstructor(false); formInstr.resetFields(); setInstrError(null); setTabInstr('manual'); setInstrFile(null);
        setIsCreateStudent(false); formStud.resetFields(); setStudError(null); setTabStud('manual'); setStudFile(null);
        break;
      default: break;
    }
  };
  ...
  const onToggleActive = async (rec: Account, checked: boolean) => {
    ...
    try {
      ...
      setUsers(curr => curr.map(u => (u.id === updated.id ? { ...u, isActive: updated.isActive } : u)));
    } catch (err) {
  ...
  const submitCreateInstructor = async () => {
    ...
    try {
      const c = await AccountService.registerInstructor({
        ...vals, gender: Number(vals.gender), isActive: true, organizationId: orgId
      });
      setUsers(u => [c, ...u]);
  ...
  const submitImportInstructor = async () => {
    if (!instrFile) { message.error('Vui lòng chọn file Excel'); return; }
    ...
    } finally { setImportingInstr(false); setInstrFile(null); }
  };
  const submitImportStudent = async () => {
    if (!studFile) { message.error('Vui lòng chọn file Excel'); return; }
  ...
  const columns: ColumnsType<Account> = [
    ...
    {
      title: "Image", dataIndex: "avtUrl", key: "avtUrl", align: "center", width: 110,
      render: (url) => url ? (
    ...
    <Layout>
      <Content style={{ padding: 24 }}>
          ...
          <Col>
            <Space wrap>
              <Button
                ...
                type="primary"
                onClick={() => { setIsCreateInstructor(true); setInstrError(null); formInstr.resetFields(); setTabInstr('manual'); setInstrFile(null); }}
              >
                Create Instructor
              </Button>
              <Button
                icon={<UserOutlined />}
                type="primary"
                onClick={() => { setIsCreateStudent(true); setStudError(null); formStud.resetFields(); setTabStud('manual'); setStudFile(null); }}
              >
                Create Student
              </Button>
            </Space>
        ...
        <Modal
          ...
          onCancel={() => { setIsCreateInstructor(false); formInstr.resetFields(); setInstrError(null); setTabInstr('manual'); setInstrFile(null); }}
          okButtonProps={{ loading: tabInstr === 'manual' ? creatingInstructor : importingInstr, disabled: tabInstr === 'manual' ? creatingInstructor : importingInstr }}
          destroyOnHidden
          width={700}
        >
          {instrError && tabInstr==='manual' && <Alert style={{ marginBottom: 12 }} type="error" showIcon message={instrError} />}

          <Tabs activeKey={tabInstr} onChange={(k)=>setTabInstr(k as any)} items={[
            {
              key: 'manual',
              label: 'Create manually',
              children: (
                <Form form={formInstr} layout="vertical" onValuesChange={() => { if (instrError) setInstrError(null); }}>
                  <Row gutter={16}>
                    <Col span={12}>
            ...
            },
            {
              ...
              children: (
                <>
                  ...
                  <Upload.Dragger
                    multiple={false} accept=".xlsx,.xls" beforeUpload={() => false} maxCount={1}
                    fileList={instrFile ? [{ uid:'-1', name: instrFile.name, status:'done' } as any] : []}
                    onChange={(info) => {
                      const f = info.fileList?.[0]?.originFileObj as File | undefined;
                      setInstrFile(f ?? null);
                    }}
                    onRemove={() => { setInstrFile(null); return true; }}
                  >
        ...
        <Modal
          ...
          onCancel={() => { setIsCreateStudent(false); formStud.resetFields(); setStudError(null); setTabStud('manual'); setStudFile(null); }}
          okButtonProps={{ loading: tabStud === 'manual' ? creatingStudent : importingStud, disabled: tabStud === 'manual' ? creatingStudent : importingStud }}
          destroyOnHidden
          width={700}
        >
          {studError && tabStud==='manual' && <Alert style={{ marginBottom: 12 }} type="error" showIcon message={studError} />}

          <Tabs activeKey={tabStud} onChange={(k)=>setTabStud(k as any)} items={[
            {
              key: 'manual',
              label: 'Create manually',
              children: (
                <Form form={formStud} layout="vertical" onValuesChange={() => { if (studError) setStudError(null); }}>
                  <Row gutter={16}>
                    <Col span={12}>
                      <Form.Item name="userName" label="Username" rules={[{ required: true }]}>
            ...
            },
            {
              ...
              children: (
                <>
                  ...
                  <Upload.Dragger
                    multiple={false} accept=".xlsx,.xls" beforeUpload={() => false} maxCount={1}
                    fileList={studFile ? [{ uid:'-1', name: studFile.name, status:'done' } as any] : []}
                    onChange={(info) => {
                      const f = info.fileList?.[0]?.originFileObj as File | undefined;
                      setStudFile(f ?? null);
                    }}
                    onRemove={() => { setStudFile(null); return true; }}
                  >
                    <p className="ant-upload-drag-icon"><InboxOutlined /></p>
                    <p className="ant-upload-text">Click or drag Excel file to this area to import</p>
                    <p className="ant-upload-hint">Accepted: .xlsx, .xls</p>
